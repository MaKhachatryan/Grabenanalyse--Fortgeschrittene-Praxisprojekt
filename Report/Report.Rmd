...
# Introduction
Understanding patterns of biological relatedness within collective burial contexts can provide insights into the social organization of past societies. In Mycenaean Greece (ca. 1500–1000 BCE), collective tombs were used over long periods and often contain the remains of many individuals. Archaeologists have long debated whether these tombs represent biological family groups, broader kin networks, or socially defined communities. Only recently has ancient DNA (aDNA) analysis made it possible to examine these questions directly.

```{r fig0, echo=FALSE, fig.cap="Selected tombs at Elateia", fig.pos='H', out.width="100%"}
knitr::include_graphics("../images/tombs.png")
```

The present study investigates patterns of genetic relatedness among individuals buried in collective tombs from the Mycenaean sites of Amfissa and Elateia. Genetic data from skeletal remains allow the identification of biological relationships and provide a new way to examine how individuals buried in the same tomb were connected.

However, analyzing relatedness in this context presents several challenges. Archaeological sampling is selective, meaning that only a subset of individuals from each tomb is genetically analyzed. In addition, DNA preservation varies across individuals and skeletal elements, affecting the quality of the genetic data. As a result, some relationships may be harder to detect. The data also have a complex structure because relatedness is observed for pairs of individuals, and each individual can appear in multiple pairwise comparisons.


The statistical analysis was guided by research questions formulated by our project partners, focusing on both tomb-level and individual-level patterns of relatedness. At the tomb level, the goal was to examine whether relatedness patterns differ between burial contexts and whether characteristics such as the duration of tomb use, the number of burials, or sampling success influence these patterns. At the individual level, the focus was on whether biological and sampling-related characteristics, such as sex, age category, or the sampled skeletal element, are associated with detected relatedness.

Overall, the study aims to evaluate how relatedness patterns vary across burial contexts while accounting for potential sources of bias in the data. By combining archaeogenetic data with statistical modeling approaches, this work contributes to a better understanding of social organization in Mycenaean collective burials.


# Data and Methods
## Data Description 
This project is based on genetic and osteological data from human remains recovered from Mycenaean collective burial contexts in Greece, dated to approximately 1500–1000 BCE. The material derives from two archaeological sites: Amfissa, which consists of a single tholos tomb, and Elateia, a large burial site containing 85 tombs. From Elateia, seven tombs were selected for detailed genetic investigation: tombs 31, 36, 46, 50, 56, 62, and 67. DNA preserved in skeletal material was extracted and analyzed to infer biological relationships among individuals buried within and between tombs.

The analysis draws on three interconnected datasets that capture information at different levels. The first dataset summarizes characteristics at the tomb level, including the minimum number of individuals identified, the number successfully analyzed genetically, the estimated duration of tomb use, the proportion of successful DNA samples, the total number of samples taken, and the distribution of sampled skeletal elements (such as petrous bones, teeth, talus or phalanx, and others). These variables highlight differences between tombs in preservation, sampling, and burial practices.

The second dataset contains metadata describing the sampled individuals. Each entry represents a single individual and records the associated tomb, estimated biological sex, age category, and the skeletal element used for DNA extraction. This information provides the biological and contextual background needed to evaluate how individual characteristics relate to genetic data quality and inferred relationships.

The third dataset reports pairwise genetic relationships between individuals based on ancient DNA analyses. For each pair, it includes estimates of genetic relatedness, the number of overlapping single nucleotide polymorphisms (SNPs), and a categorical classification of relationship degree. Since individuals can appear in multiple pairwise comparisons, observations in this dataset are not independent, resulting in a complex relational structure.
...
As shown in Figure 3, classification accuracy is strongly dependent on genome coverage. At low coverage levels, false positive rates increase substantially, particularly for third-degree relationships. On the contrary, the power to correctly identify true relatives rises sharply as the number of overlapping SNPs increases.
...
```{r fig5, echo=FALSE, fig.cap="Directed Acyclic Graph (DAG)", fig.pos='H', out.width="95%"}
...
### Descriptive Plots
To obtain an initial overview of kinship patterns across burial sites, we analysed both the absolute number and the proportion of within-tomb pairs by relationship category (related, unrelated, uncertain) (Figures 6 and 7).
...
Figure 6 shows the absolute number of within-tomb pairs classified as related, unrelated, or uncertain. As expected, the total number of pairs varies substantially across tombs, reflecting differences in sample size.
...
To account for differences in tomb size, Figure 7 presents the proportion of within-tomb pairs by relationship category.
...
In the remaining tombs, the networks are generally smaller and more fragmented. Most consist of small clusters of related individuals connected by one or two ties. Cross-tomb links are again visible in several cases, while strong within-tomb clustering is limited. (see [Appendix](#appendix))
...
1. Multiple Membership of Individuals: Each individual appears in multiple pairs. For example, if individual $i$ is compared to $j$, $k$, and $l$, the errors associated with those three observations will be correlated because they all share the unique genetic and environmental traits of individual $i$.

2. Tomb-Level Dependencies: Pairs can exist both within the same tomb or across different tombs. Because individuals within a tomb may share common environmental or social factors, we must account for the random "tomb effect." A pair ${i,j}$ effectively "belongs" to the tombs $T(i)$ and $T(j)$.
...
The largest difference is observed for burial context: pairs buried in the same tomb have predicted probabilities of 2.20% and 2.40%, compared to 0.22% and 0.19% for pairs from different tombs. Additionally the Grouped Model has much wider 95% credible intervals than the Separated Model across all predictors. This potentially reflects higher statistical uncertainty caused by reducing the number of independent tomb clusters from eight to six.
...
```{r a2, echo=FALSE, fig.cap="Estimations of model with flat prior and with weakly informative prior", fig.pos='H', out.width="90%"}
...
```{r a3, echo=FALSE, fig.cap="Estimations of main model and 'with-in tomb' model", fig.pos='H', out.width="90%"}
...
```{r ap network, echo=FALSE, fig.cap="Network plots of Elateia Tombs", fig.pos='H', out.width="100%"}
knitr::include_graphics("../Plots/Elateia_T62_network.png")
knitr::include_graphics("../Plots/Elateia_T56_network.png")
knitr::include_graphics("../Plots/Elateia_T50_network.png")
knitr::include_graphics("../Plots/Elateia_T46_network.png")
knitr::include_graphics("../Plots/Elateia_T36_network.png")
knitr::include_graphics("../Plots/Elateia_T31_network.png")
knitr::include_graphics("../Plots/Elateia_T67_network.png")
```
