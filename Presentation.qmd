...
### Proposal: Include Overlap and Low_data in our model
...
**Include both overlap and low_data in the relatedness model** so the model explicitly accounts for the mechanism that creates uncertainty.

Even if two pairs have identical overlap, **their threshold differs by degree**, and the low_data indicator lets the model learn this.
:::
...
-   Multi-Membership Generalized Linear Model(GLMM) (We reuse the model structure from the main analysis) with random effects for both 2 tombs and 2 individuals
-   Right now only one version for all separated tombs
...
![](images/Screenshot%202025-12-07%20at%2005.13.48.png){width="645"}

## Proposal to handle MNAR

### Interpreting the Low Bone Quality Coefficient
...
In our DAG, several archaeological and biological factors (age, sex, skeletal element, sample success, tomb use length) directly influence **DNA preservation**.

DNA preservation determines how many SNPs overlap between two individuals:
...
-   **Overlap is the key data-quality variable** in our dataset.

-   It is the variable that triggers uncertainty (MNAR) in kinship labels.

-   To correctly adjust for this bias, **we first need to understand what predicts overlap itself.**
:::
...
Overlap SNPs is a continuous, approximately symmetric variable after standardization, we model it with a Gaussian GLM with an identity link. This is equivalent to a linear regression model, but framed as a generalized linear model for consistency with our other models.
...
1.  Lower bone quality strongly reduces overlap

2.  Pairs coming from tombs with higher sampling success show better DNA overlap.

3.  Subadults tend to yield higher overlap

4.  Being buried in the same tomb has only a negligible influence on overlap

    ::: fragment
    $\Rightarrow$ DNA preservation (overlap) is mainly driven by bone quality and tomb-level sampling success.

    It shows that overlap is predictable, structured, and therefore the right variable to include when correcting MNAR bias in the relatedness model.
    :::
